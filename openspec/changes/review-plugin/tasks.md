...
- [x] 1.1 Create `packages/agent-use/` with `package.json` (`@giantanalyticsai/docx-editor-agents`, BSL-1.1, peer dep on `@giantanalyticsai/docx-core`), `tsconfig.json`, `tsup.config.ts` with two entry points (`src/index.ts`, `src/bridge.ts`)
