...
Monorepo: `packages/core` (`@giantanalyticsai/docx-core`, MIT), `packages/react`, `packages/vue`. Root workspace `"packages/*"`. Build: tsup. New package: `packages/agent-use` → `@giantanalyticsai/docx-editor-agents` (BSL-1.1).
...
`packages/agent-use/` with `@giantanalyticsai/docx-core` as peer dep. Published on public npm under BSL-1.1.
