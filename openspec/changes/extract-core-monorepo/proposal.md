...
- **BREAKING**: Split the single `@giantanalyticsai/docx-js-editor` package into a Bun workspaces monorepo with separate packages
- New `@giantanalyticsai/docx-core` package containing all framework-agnostic code: `src/docx/`, `src/types/`, `src/prosemirror/`, `src/layout-engine/`, `src/layout-painter/`, `src/layout-bridge/`, `src/core-plugins/`, `src/utils/` (non-React parts), `src/agent/`, `src/mcp/`
- `@giantanalyticsai/docx-js-editor` becomes a thin React UI package depending on `@giantanalyticsai/docx-core`, containing: `src/components/`, `src/hooks/`, `src/plugin-api/`, `src/plugins/`, `src/paged-editor/`
- Remove React type leaks from core code paths (e.g., `CSSProperties` imports in utils)
- Split `EditorPlugin` interface into a framework-agnostic core (`EditorPluginCore` in `@giantanalyticsai/docx-core`) and framework-specific adapters (`ReactEditorPlugin` in React package, `VueEditorPlugin` in Vue package)
- Existing `@giantanalyticsai/docx-js-editor` keeps its npm name and download stats — existing users continue using it as before
...
- `core-package-api`: Public API surface for `@giantanalyticsai/docx-core` — exports for DOCX parsing/serialization, ProseMirror schema/extensions/commands, layout engine, document model types, and headless editor creation
...
- **npm**: New `@giantanalyticsai/docx-core` package published; `@giantanalyticsai/docx-js-editor` gains `@giantanalyticsai/docx-core` as dependency; `@giantanalyticsai/docx-editor-vue` scaffolded for community contribution
...
- **Downstream**: Vue/Svelte/vanilla consumers can depend on `@giantanalyticsai/docx-core` directly with zero React overhead
