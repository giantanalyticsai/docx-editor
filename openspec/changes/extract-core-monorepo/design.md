...
The editor is currently a single npm package (`@giantanalyticsai/docx-js-editor`) with ~80% framework-agnostic code and ~20% React UI. The codebase already has clean internal boundaries — separate entry points (`core.ts`, `headless.ts`, `react.ts`, `ui.ts`) and framework-agnostic directories (`src/docx/`, `src/types/`, `src/prosemirror/`, `src/layout-engine/`, `src/layout-painter/`, `src/layout-bridge/`). A community contributor wants to build a Vue wrapper, which requires the core to be importable without React dependencies.
...
- Extract framework-agnostic core into `@giantanalyticsai/docx-core` with zero React dependencies
- Keep `@giantanalyticsai/docx-js-editor` as the React UI package (preserves npm stats, no migration for existing users)
- Use Bun workspaces for monorepo management (already using Bun as runtime)
- Enable community Vue/Svelte wrappers that depend only on `@giantanalyticsai/docx-core`
...
**Decision:** Split into exactly `@giantanalyticsai/docx-core` and `@giantanalyticsai/docx-js-editor` (React).
...
**Decision:** `@giantanalyticsai/docx-js-editor` stays as the React package name. No rename.
...
**Core (`@giantanalyticsai/docx-core`):**
...
**React (`@giantanalyticsai/docx-js-editor`):**
...
**Decision:** React package imports from `@giantanalyticsai/docx-core` (the npm package name), not relative paths.
...
**Core interface (`@giantanalyticsai/docx-core`):**
...
**React adapter (`@giantanalyticsai/docx-js-editor`):**
...
**Vue adapter (`@giantanalyticsai/docx-editor-vue`):**
...
- `EditorPluginCore`, `PanelConfig`, `PluginPanelProps` (without ReactNode), `RenderedDomContext` → `@giantanalyticsai/docx-core`
...
**[Risk] Internal import churn** → Files that currently do `../docx/parser` will need `@giantanalyticsai/docx-core` imports. Mitigation: batch find-and-replace, verify with typecheck.
