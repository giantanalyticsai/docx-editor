...
The `packages/core/` directory SHALL contain all framework-agnostic source code with its own `package.json` named `@giantanalyticsai/docx-core`.
...
The `packages/react/` directory SHALL contain all React-dependent source code with its own `package.json` named `@giantanalyticsai/docx-js-editor`.
...
- **THEN** `@giantanalyticsai/docx-core` SHALL be listed in dependencies
...
- **AND** it SHALL externalize `@giantanalyticsai/docx-core` (not bundle it)
...
- **THEN** tests SHALL import from both `@giantanalyticsai/docx-core` and `@giantanalyticsai/docx-js-editor`
...
- **AND** the app SHALL render a functional DOCX editor using `@giantanalyticsai/docx-editor-vue` and `@giantanalyticsai/docx-core`
...
The `packages/vue/` directory SHALL contain an empty scaffolded package named `@giantanalyticsai/docx-editor-vue` with a dependency on `@giantanalyticsai/docx-core`, ready for community contribution.

#### Scenario: Vue package has minimal package.json

- **WHEN** inspecting `packages/vue/package.json`
- **THEN** it SHALL have name `@giantanalyticsai/docx-editor-vue`
- **AND** `@giantanalyticsai/docx-core` SHALL be listed in dependencies
