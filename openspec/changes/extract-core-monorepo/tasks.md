...
- [x] 1.3 Create `packages/core/package.json` with name `@giantanalyticsai/docx-core`, dependencies (prosemirror-\*, jszip, pizzip, xml-js, docxtemplater, clsx), and exports config
- [x] 1.4 Create `packages/react/package.json` with name `@giantanalyticsai/docx-js-editor`, dependency on `@giantanalyticsai/docx-core`, peerDependencies (react, react-dom), and `@radix-ui/react-select`
- [x] 1.5 Create `packages/vue/package.json` with name `@giantanalyticsai/docx-editor-vue`, dependency on `@giantanalyticsai/docx-core`, peerDependency on `vue`, and a placeholder `src/index.ts`
...
- [x] 5.3 Export `EditorPluginCore`, `PluginPanelProps`, `PanelConfig`, `RenderedDomContext` from `@giantanalyticsai/docx-core`
...
- [x] 5.8 Export `ReactEditorPlugin` from `@giantanalyticsai/docx-js-editor` and `VueEditorPlugin` from `@giantanalyticsai/docx-editor-vue`

## 6. Update Internal Imports

- [x] 6.1 Update all imports in `packages/react/` that reference core modules to use `@giantanalyticsai/docx-core` instead of relative paths
- [x] 6.2 Update `packages/react/src/index.ts` to re-export everything from `@giantanalyticsai/docx-core` for backwards compatibility
...
- [x] 7.2 Create `packages/react/tsup.config.ts` with entry points (`index.ts`, `react.ts`, `ui.ts`, re-exports) externalizing `@giantanalyticsai/docx-core`
...
- [x] 10.1 Verify `@giantanalyticsai/docx-js-editor` main export includes all previously exported symbols
- [x] 10.2 Verify subpath exports (`/core`, `/headless`, `/react`, `/ui`, `/core-plugins`, `/mcp`) still work from the React package
- [x] 10.3 Verify `@giantanalyticsai/docx-core` can be imported and used without React installed
