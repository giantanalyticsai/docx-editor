...
A Vue contributor would need to reimplement all of this logic. The goal is to extract the state machines and coordination logic into framework-agnostic classes in `@giantanalyticsai/docx-core`.

## Goals / Non-Goals

**Goals:**

- Extract framework-agnostic state machines and coordination logic from React components into plain TypeScript classes in `@giantanalyticsai/docx-core`
...
- Changing any public API of `@giantanalyticsai/docx-js-editor`
...
### 4. Keep managers in `@giantanalyticsai/docx-core` as plain classes

**Decision:** All manager/coordinator classes live in `packages/core/src/managers/` and are exported from `@giantanalyticsai/docx-core`.
